...
// import idlImported from '$lib/idl/solblog.json';
const idl = JSON.parse(require('fs').readFileSync('./idl/solblog.json', 'utf8'));
...
export default class AnchorClient {
	...
	async initialize(bio) {
		...
		console.log('blogAccount.publicKey.toString()', blogAccount.publicKey.toString());
	...
	async getLatestPost(blogAccount) {
		const account = await this.program.account.blogAccount.fetch(
			new anchor.web3.PublicKey(blogAccount.publicKey)
		);
